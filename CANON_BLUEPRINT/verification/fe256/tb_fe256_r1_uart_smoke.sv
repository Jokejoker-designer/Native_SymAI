// D-owned UART smoke of FE256_HW_R1 on shadow-bind candidate.
// Exercises uart_rx_word -> uart_fe256_host -> fe256_query_path -> uart_tx_word.
// Does not force tb_steer. B gold hex only; B TB not modified.
// CANDIDATE. PROGRAM=NO. XSim != board. Not FE256_PASS / BOARD_PASS.
`timescale 1ns/1ps
`include "fe256_abi_constants.svh"

module tb_fe256_r1_uart_smoke;
  ...
  integer fd, code, rec, bi, hi, lo, wi, t, mismatch, ci, cidx;
  string line;
  ...
  initial begin
    ...
    for (ci = 0; ci < N_SMOKE; ci++) begin
      ...
      if (mismatch != 0) begin
        ...
        mismatch = 0;
        for (bi = 0; bi < RW; bi++) begin
          if (got_r[bi] !== gold_r[cidx][bi]) mismatch++;
        end
        if (mismatch != 0) begin
          n_fail++;
          $display("UART_FE256_MISMATCH case %0d byte_mismatches=%0d", cidx, mismatch);
        end else begin
          n_pass++;
          $display("UART_FE256_MATCH case %0d", cidx);
        end
      end
    ...
    if (n_fail == 0 && n_pass == N_SMOKE)
      $display("UART_FE256_XSIM_SMOKE %0d/%0d bit-exact (simulation only; not FE256_PASS; not BOARD_PASS)",
               n_pass, N_SMOKE);
    else
      $display("UART_FE256_XSIM_SMOKE_FAIL pass=%0d fail=%0d", n_pass, n_fail);
    $finish;
  end
endmodule
