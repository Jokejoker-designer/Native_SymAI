# Synth current arty_a7_mig_top (Q*/SPEAR/walk on ui_clk). CANDIDATE. PROGRAM=NO.
# Separate out dir: do not clobber mig_bind/post_route.dcp (pre-ui_clk-move route).
# Not MIG_PASS / BOARD_PASS / TIMING_PASS.
...
set out {D:/FPGA/arty_d/mig_uiclk}
file mkdir $out
file mkdir [file join $out reports]
cd $out

create_project mig_uiclk $out -part xc7a100tcsg324-1 -force
...
read_verilog -sv [file join $rtl board arty_a7_mig_top.sv]
...
synth_design -top arty_a7_mig_top -part xc7a100tcsg324-1 -flatten_hierarchy rebuilt
...
puts "ARTY_MIG_UICLK_SYNTH_DONE"
exit
