...
$sim = Join-Path $Root "vivado\m2_directory\xsim"
...
copy /Y `"$dir\dir_keys.hex`" `"$sim\`" >nul
call xvlog -sv `"$dir\exact_directory.sv`" `"$tb`"
...
call xsim tb_m2_dir_snap -tclbatch run_all.tcl -log `"$sim\xsim.log`"
if errorlevel 1 exit /b 1
"@
$bat = Join-Path $sim "run_xsim.bat"
Set-Content -Path $bat -Value $runSim -Encoding ASCII
cmd /c $bat
if ($LASTEXITCODE -ne 0) { throw "M2 directory XSim failed: $LASTEXITCODE" }
