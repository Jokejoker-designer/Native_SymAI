...
$sim = Join-Path $Root "vivado\m2_posting\xsim"
...
call xvlog -sv `"$dir\exact_directory.sv`" `"$dir\posting_walk.sv`" `"$tb`"
...
call xsim tb_m2_post_snap -tclbatch run_all.tcl -log `"$sim\xsim.log`"
if errorlevel 1 exit /b 1
"@
$bat = Join-Path $sim "run_xsim.bat"
Set-Content -Path $bat -Value $runSim -Encoding ASCII
cmd /c $bat
if ($LASTEXITCODE -ne 0) { throw "M2 posting XSim failed: $LASTEXITCODE" }
