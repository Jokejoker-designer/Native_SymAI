...
// UG901 sync 1R BRAM for post[]. PostingEntry.edge_ref is a T2 byte address.
`timescale 1ns/1ps
...
module posting_walk #(
  ...
  logic [11:0]  post_addr;
  logic [127:0] post_q;
  always_ff @(posedge clk) begin
    post_q <= post[post_addr];
  end
  ...
  typedef enum logic [2:0] {
    S_IDLE, S_DIR, S_HDR_WAIT, S_HDR, S_ENT_WAIT, S_ENT, S_RSP
  } state_t;
  state_t state;
          ...
          if (d_rsp) begin
            ...
            if (!d_hit) begin
              ...
              if ((rev_r ? rev_ptr : fwd_ptr) == 32'h0) begin
                ...
                post_addr <= rev_r ? rev_ptr[15:4] : fwd_ptr[15:4];
                state <= S_HDR_WAIT;
              end
            end
          end
        end
        S_HDR_WAIT: state <= S_HDR;
        S_HDR: begin
          if (post_q[31:0] != sid) begin
            mism <= 1'b1;
            found <= 1'b0;
            state <= S_RSP;
          end else begin
            cnt <= post_q[79:64];
            found <= 1'b1;
            if (post_q[79:64] == 16'h0) state <= S_RSP;
            else begin
              widx <= widx + 12'h1;
              post_addr <= widx + 12'h1;
              state <= S_ENT_WAIT;
            end
          end
        end
        S_ENT_WAIT: state <= S_ENT;
        S_ENT: begin
          eref0 <= post_q[31:0];
          nb0 <= post_q[63:32];
          state <= S_RSP;
