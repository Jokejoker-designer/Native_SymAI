...
OBSERVATION:
  ...
  FACT — Top-level 59 directories + 1 file (Bang Ke QUY 3 2025.pdf). GciErrorCount=0.
  ...
  FACT — Filename/folder scan for hdld, hdtv, lao dong, thu viec, employment, contract, apprenticeship, intern, personnel, HR: 0 hits.
  FACT — 46 files match nhan vien (site worker registration lists under Bao gia, not Hop dong lao dong).
  FACT — 1 file LaborLikely=true via substring nhansu inside "xac nhan sua chua" (false positive).
  INFERENCE — Tree is HVAC/construction project archive; Hop dong folders are commercial installation/sale/maintenance contracts.
  INFERENCE — No employee labor contract is present under the requested name/folder criteria.
HYPOTHESES:
  H1 Labor contracts exist but use only person names / HD + person without HDLD keywords. UNTESTED (content read disallowed).
  H2 Labor contracts live outside D:\2025. UNKNOWN.
  H3 Empty Hop dong folders (29) once held files that were moved. HYPOTHESIS.
HOW_TRACE:
...
DECISIVE_TEST:
  Recurse name+parent match including hdld, hopdonglaodong, employment, contract: 0 labor-contract names.
ROOT_CAUSE_OR_UNKNOWN:
  FACT: no HDLD-named files in D:\2025. UNKNOWN whether contracts exist under unrelated names.
REUSABLE_DECISION_PROCEDURE:
  Confirm root. Recurse Get-ChildItem with ErrorVariable. Normalize Vietnamese then phrase+compact match. Whole-token short keys. Split commercial hop dong vs labor-strong. Do not treat luong or nhan su as substring of khoi luong / xac nhan sua. Never open contents unless authorized.
STRUCTURAL_GUARD:
...
VERDICT_BY_LAYER:
  PASS_IMPLEMENTED name-index of D:\2025. NOT a labor-contract archive finding.
LESSON_TO_SHARE:
  LESSON_ID FILESEARCH-SUBSTRING-FALSE-POSITIVE-NHANSU-KHOILUONG-20260917T095900Z
NEXT_DECISIVE_EXPERIMENT:
  If owner authorizes content search, text-extract hop dong plus HD*.pdf; else search other drives for HDLD.
