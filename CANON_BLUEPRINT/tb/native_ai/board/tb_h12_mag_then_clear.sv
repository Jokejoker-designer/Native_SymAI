// tb_h12_mag_then_clear.sv — after MAG (extra after BEGIN), CLEAR vs pad3 then CLEAR.
// Dest BRAM. Not PACK_ABI_24_24_PASS / not BOARD_PASS.
`timescale 1ns/1ps

module tb_h12_mag_then_clear;
  ...
  int nwords, i, c;
  logic [31:0] got, mag_clear, pad_clear;
  int mag_st, pad_st;
  ...
  task automatic mag_pack();
    begin
      send_word(CLR_CMD);
      wait_word(got, 400000);
      $display("MTC CLEAR_PRE got=%08h", got);
      send_word(vec[0]);
      uart_byte(8'h00);
      for (i = 1; i < nwords; i++)
        send_word(vec[i]);
      wait_word(got, 3_000_000);
      $display("MTC PACK_MAG got=%08h expect=%08h ld_st=%0d qsc=%0b bix=%0d",
               got, MAG, u_h.u_ld.u_ld.state, u_h.qsc_ui, u_h.u_rx.bix);
    end
  endtask
  ...
  initial begin
    ...
    mag_pack();
    send_word(CLR_CMD);
    wait_word(mag_clear, 400000);
    mag_st = u_h.u_ld.u_ld.state;
    $display("MTC MAG_THEN_CLEAR got=%08h expect_ACK=%08h ld_st=%0d qsc=%0b bix=%0d",
             mag_clear, CLR_ACK, mag_st, u_h.qsc_ui, u_h.u_rx.bix);
    ...
    uart_byte(8'h00);
    send_word(CLR_CMD);
    wait_word(pad_clear, 400000);
    pad_st = u_h.u_ld.u_ld.state;
    $display("MTC MAG_PAD3_THEN_CLEAR got=%08h (0=MUTE) ld_st=%0d qsc=%0b bix=%0d",
             pad_clear, pad_st, u_h.qsc_ui, u_h.u_rx.bix);

    if (mag_clear === 32'h0 && pad_clear === CLR_ACK)
      $display("H12_MAG_THEN_CLEAR_XSIM_LEFTOVER_BIX1_PAD3_ACK (not PACK_ABI_24_24_PASS / not BOARD_PASS)");
    else
      $display("H12_MAG_THEN_CLEAR_XSIM_FAIL mag_clear=%08h pad_clear=%08h", mag_clear, pad_clear);
    $finish;
  end
endmodule
