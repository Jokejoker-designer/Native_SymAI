// tb_h12_mag_resync.sv — MAG from stray byte before PACK; host pad-resync.
// Does not edit uart_rx_word. Dest BRAM. Not PACK_ABI_24_24_PASS / not BOARD_PASS.
`timescale 1ns/1ps

module tb_h12_mag_resync;
  ...
  task automatic gold_once();
    begin
      send_word(CLR_CMD);
      wait_word(got, 400000);
      $display("MAGRS CLEAR1 got=%08h", got);
      for (i = 0; i < nwords; i++)
        send_word(vec[i]);
      wait_word(got, 3_000_000);
      $display("MAGRS PACK1 got=%08h", got);
    end
  endtask
  ...
  initial begin
    ...
    for (i = 0; i < nwords; i++)
      send_word(vec[i]);
    wait_word(got, 3_000_000);
    $display("MAGRS stray1_then_PACK got=%08h expect_MAG=%08h ld_st=%0d",
             got, MAG, u_h.u_ld.u_ld.state);
    ...
    send_word(CLR_CMD);
    wait_word(got, 400000);
    $display("MAGRS stray1_CLEAR got=%08h", got);
    uart_byte(8'h00);
    uart_byte(8'h00);
    uart_byte(8'h00);
    wait_word(got, 400000);
    $display("MAGRS pad3 junk tok got=%08h", got);
    send_word(CLR_CMD);
    wait_word(got, 400000);
    $display("MAGRS resync CLEAR got=%08h", got);
    for (i = 0; i < nwords; i++)
      send_word(vec[i]);
    wait_word(got, 3_000_000);
    $display("MAGRS resync PACK got=%08h", got);

    if (got === GOLD)
      $display("H12_MAG_RESYNC_XSIM_PACK_GOLD_AFTER_PAD (not PACK_ABI_24_24_PASS / not BOARD_PASS)");
    else
      $display("H12_MAG_RESYNC_XSIM_FAIL resync_pack=%08h", got);
    ...
    for (i = 1; i < nwords; i++)
      send_word(vec[i]);
    wait_word(got, 3_000_000);
    $display("MAGRS extra_after_BEGIN PACK got=%08h expect_MAG=%08h", got, MAG);
    $finish;
  end
endmodule
