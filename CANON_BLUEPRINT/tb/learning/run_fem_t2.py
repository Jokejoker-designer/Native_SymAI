"""FEM dest-stall adapter XSim. Not FEM_PERSIST_PASS."""
...
TB = ROOT / "tb" / "learning" / "tb_fem_t2_adapter.v"
BUILD = ROOT / "tb" / "learning" / "build" / "fem_t2"
TOP = "tb_fem_t2_adapter"
MARK = "FEM_T2_ADAPTER_XSIM_PASS"
...
def main() -> int:
    ...
    for cmd in (["xvlog"] + srcs, ["xelab", TOP, "-s", "snap_t2"]):
        rc, log = run(cmd, cwd=BUILD)
        if rc:
            print("COMPILE_FAIL\n" + log)
            return 1
    (BUILD / "run_all.tcl").write_text("run -all\nexit\n", encoding="ascii")
    rc, log = run(["xsim", "snap_t2", "-tclbatch", "run_all.tcl"], cwd=BUILD)
    print(log)
