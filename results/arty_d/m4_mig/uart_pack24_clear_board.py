...
def find_known(raw: bytes) -> tuple[int | None, str]:
    """Return ACK/BUSY/ERR if present. Do not treat first-4 garbage as a token."""
    hexdump = raw.hex()
    for tok in (CLR_ACK, CLR_BUSY, CLR_ERR):
        if raw.find(tok.to_bytes(4, "little")) >= 0:
            return tok, hexdump
    return None, hexdump


def do_clear(ser, retries: int = 1, capture_s: float = 3.0) -> tuple[int | None, str]:
    last_hex = ""
    last: int | None = None
    for attempt in range(max(1, retries)):
        drain_until_idle(ser, idle_s=0.15, max_s=1.0)
        send_words(ser, [CLR_CMD])
        raw = read_raw(ser, capture_s, idle_s=0.15)
        got, hexdump = find_known(raw)
        last_hex = hexdump
        print(
            f"CLEAR attempt={attempt} n={len(raw)} hex={hexdump[:128]}"
            f"{'...' if len(hexdump) > 128 else ''} tok="
            f"{None if got is None else f'{got:08x}'}",
            flush=True,
        )
        if got in (CLR_ACK, CLR_BUSY, CLR_ERR):
            drain_until_idle(ser, idle_s=0.1, max_s=0.4)
            return got, hexdump
        last = got
        time.sleep(0.05)
    drain_until_idle(ser, idle_s=0.1, max_s=0.5)
    return last, last_hex


def run_clear_rounds(
    ser, rows: list[dict], rounds: int, retries: int = 1
) -> list[dict]:
    recs: list[dict] = []
    for rnd in range(rounds):
        for row in rows:
            ...
            cg, clr_hex = do_clear(ser, retries=retries, capture_s=3.0)
            if cg != CLR_ACK:
                rec = {
                    "round": rnd,
                    "case_id": cid,
...
def main() -> int:
    ...
    ap.add_argument("--mode", choices=["clear", "seq", "compare", "probe"], default="clear")
    ap.add_argument("--rounds", type=int, default=2)
    ap.add_argument("--clear-retries", type=int, default=1)
    args = ap.parse_args()
    port = find_port()
    if port is None:
        print("SKIP no COM")
    ...
    try:
        if args.mode == "probe":
            cg, hexdump = do_clear(ser, retries=1, capture_s=3.0)
            print(f"PROBE tok={None if cg is None else f'{cg:08x}'} n={len(hexdump)//2} hex={hexdump}")
            (EVID / "UART_CLEAR_PROBE.txt").write_text(
                f"tok={None if cg is None else f'{cg:08x}'}\nhex={hexdump}\n",
                encoding="utf-8",
            )
            return 0 if cg == CLR_ACK else 1
        if args.mode == "clear":
            recs = run_clear_rounds(
                ser, rows, args.rounds, retries=args.clear_retries
            )
