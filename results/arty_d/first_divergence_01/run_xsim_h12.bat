...
set TB=%ROOT%\tb\native_ai\board
set XD=D:\FPGA\arty_d\first_divergence_01\xsim_h12
...
call xelab tb_h12_class_a -snapshot snap_h12 -timescale 1ns/1ps
if errorlevel 1 exit /b 1
echo run -all> "%XD%\run_all.tcl"
echo exit>> "%XD%\run_all.tcl"
call xsim snap_h12 -tclbatch run_all.tcl -log "%XD%\xsim.log"
if errorlevel 1 exit /b 1
