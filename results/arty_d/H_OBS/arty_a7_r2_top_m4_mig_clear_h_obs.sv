// H_OBS UART-dump observe identity. H_OBS != identity H cf62102f.
// Reproducer with internal first_pack_word + bix. Does not explain identity H.
// BASIC license: no create_debug_core. Do not overwrite H / freeze DCPs.
// Not BOARD_PASS / PROGRAM_PASS / PACK_ABI_24_24_PASS / TIMING_PASS / MIG_PASS.
`timescale 1ns/1ps

module arty_a7_r2_top_m4_mig_clear_h_obs (
  ...
  logic [31:0] f_data;
  logic dump_take, dump_valid, dump_ready, dump_go;
  logic [31:0] dump_data;
  logic [1:0] dbg_bix, dbg_st;
  logic [7:0] dbg_sh;
  logic dbg_rx_d;
  logic [3:0] dbg_bitn;
  logic cap_fresh, cap_have, cap_drop;
  logic [7:0] cap_sh0, cap_sh1, cap_sh2, cap_sh3;
  logic [3:0] cap_n;
  logic [1:0] cap_bix_arm, cap_bix_fp, cap_bix_gap;
  logic [31:0] cap_word;
  wire pack_op = (f_data[7:0] == 8'h01);
  wire is_dump_word = w_valid && (w_data == 32'h504D5544);
  wire fifo_wr_valid = w_valid && !clr_take && !is_dump_word && !dump_take && !clr_hold;

  (* ASYNC_REG = "TRUE" *) logic req_u0, req_u1, ack_c0, ack_c1, nack_c0, nack_c1, qsc_c0, qsc_c1;
  logic qsc_ui_r;
  (* DIRECT_RESET = "yes" *) logic rst100_pack_n, rst_ui_pack_n;
  ...
  h_obs_cap u_cap (
    .clk(clk100), .rst_n(rst100_n),
    .clr_take, .w_valid, .w_ready, .w_data,
    .fifo_wr_valid, .fifo_wr_ready, .dump_go,
    .dbg_bix, .dbg_sh, .dbg_st,
    .cap_fresh, .have_pack_word(cap_have), .first_pack_word(cap_word),
    .bix_at_arm(cap_bix_arm), .bix_at_first_pack(cap_bix_fp), .bix_gap(cap_bix_gap),
    .drop_seen(cap_drop),
    .sh0(cap_sh0), .sh1(cap_sh1), .sh2(cap_sh2), .sh3(cap_sh3), .n_bytes(cap_n)
  );

  h_obs_dump u_dump (
    .clk(clk100), .rst_n(rst100_n),
    .go(dump_go),
    .in_valid(w_valid), .in_data(w_data), .take(dump_take),
    .out_valid(dump_valid), .out_ready(dump_ready), .out_data(dump_data),
    .have_pack_word(cap_have), .cap_fresh, .drop_seen(cap_drop),
    .bix_at_arm(cap_bix_arm), .bix_at_first_pack(cap_bix_fp), .bix_gap(cap_bix_gap),
    .n_bytes(cap_n),
    .sh0(cap_sh0), .sh1(cap_sh1), .sh2(cap_sh2), .sh3(cap_sh3),
    .first_pack_word(cap_word)
  );

  word_fifo32 #(.DEPTH(128)) u_rfifo (
    .clk(clk100), .rst_n(rst100_n),
    .wr_valid(fifo_wr_valid), .wr_ready(fifo_wr_ready), .wr_data(w_data),
  ...
  always_ff @(posedge clk100 or negedge rst100_n) begin
    if (!rst100_n)
      ...
      dump_go <= st_valid_100 && st_ready_100 && cap_fresh;
  end
  assign mux_valid = clr_ack_valid | dump_valid | uart_tx_valid | st_valid_100;
  assign mux_data  = clr_ack_valid ? clr_ack_data :
                     (dump_valid ? dump_data :
                     (uart_tx_valid ? uart_tx_data : st_data_100));
  assign clr_ack_ready = mux_ready && !uart_flush;
  assign dump_ready    = (!clr_ack_valid) && dump_valid && mux_ready;
  assign uart_tx_ready = (!clr_ack_valid) && (!dump_valid) && uart_tx_valid && mux_ready;
  assign st_ready_100  = (!clr_ack_valid) && (!dump_valid) && (!uart_tx_valid) && mux_ready;
