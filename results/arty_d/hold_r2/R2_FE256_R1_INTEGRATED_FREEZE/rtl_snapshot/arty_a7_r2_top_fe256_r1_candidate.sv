// Isolated FE256_HW_R1 shadow-bind candidate. PROGRAM=NO.
// Do not overwrite frozen arty_a7_r2_top / R2_TOP_ROUTE_BASELINE_WHS_0P021.
`timescale 1ns/1ps

module arty_a7_r2_top_fe256_r1_candidate (
  ...
  logic [7:0]  fe_q_bytes [0:31];
  logic [7:0]  fe_r_bytes [0:47];
  logic        fe_q_valid, fe_q_ready, fe_r_valid, fe_r_ready;
  ...
  uart_fe256_host u_qhost (
    ...
    .r_valid(fe_r_valid), .r_ready(uart_r_ready), .r_bytes(fe_r_bytes),
  ...
  always_comb begin
    for (tbi = 0; tbi < 32; tbi++)
      fe_q_bytes[tbi] = tb_steer ? tb_q_pack[8*tbi +: 8] : uart_q_bytes[tbi];
  end
  assign fe_q_valid   = tb_steer ? tb_q_valid : uart_q_valid;
  assign uart_q_ready = tb_steer ? 1'b0 : fe_q_ready;
  assign fe_r_ready    = tb_steer ? tb_r_ready : uart_r_ready;

  (* keep_hierarchy = "yes" *)
  fe256_query_path u_fe256 (
    .clk, .rst_n,
    .q_valid(fe_q_valid), .q_ready(fe_q_ready), .q_bytes(fe_q_bytes),
    .r_valid(fe_r_valid), .r_ready(fe_r_ready), .r_bytes(fe_r_bytes)
  );
  ...
  assign led = {load_reject, load_ack, fe_r_valid, q_pending};
endmodule
