// UART_R2_U24 overlay. Do not patch frozen U20/U22. PACKAGE live top NOT overwritten.
// U20 GOLD works; 24/24 FAIL_BOARD V-04 UNSUP after leftover LSB=0x01 payload.
// U21 fifo_flush on pack_lock fall killed GOLD. U24 does not flush on unlock.
// Unlocked steer is exact ABI BEGIN 00800001 only (not every opcode 0x01).
// U20 CLEAR one-cycle S_DROP flush kept. TX.flush=uart_flush. CANDIDATE.
...
module arty_a7_r2_top_m4_mig_candidate (
  ...
  wire steer_pack = pack_lock || pack_begin;
  ...
  uart_fe256_host u_qhost (
    ...
    .in_valid(f_valid && !clr_take && !clr_hold && !pack_lock && !pack_begin),
  ...
  always_ff @(posedge clk100 or negedge rst100_n) begin
    if (!rst100_n)
      ...
      pack_lock <= 1'b0;
    else if (f_valid && f_ready && !q_taking && pack_begin)
      pack_lock <= 1'b1;
  end
