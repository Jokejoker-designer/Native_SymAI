// tb_u17_harness_repeat.sv — U17 dualclk GOLD then CLEAR x32. Dest=mig_ui_bram not mig0.
// BAUD 1M. Not BOARD_PASS.
`timescale 1ns/1ps

module tb_u17_harness_repeat;
  ...
  task automatic expect_one(input logic [31:0] want, input string tag);
    begin
      wait_word(got, 400000, mute);
      if (mute || got !== want) begin
        $display("UART_R2_U17H_FAIL %s mute=%0d got=%08h want=%08h rnd=%0d", tag, mute, got, want, rnd);
        $finish;
      end
      wait_word(got, 20000, mute);
      if (!mute) begin
        extra += 1;
        $display("UART_R2_U17H_FAIL extra after %s got=%08h rnd=%0d", tag, got, rnd);
        $finish;
      end
    end
  endtask
  ...
  initial begin
    ...
    $display("U17H nwords=%0d DIV=%0d", nwords, DIV);

    send_word(CLR_CMD);
    expect_one(CLR_ACK, "CLEAR1");
    for (i = 0; i < nwords; i++)
      send_word(vec[i]);
    wait_word(got, 2_000_000, mute);
    if (mute || got !== GOLD) begin
      $display("UART_R2_U17H_FAIL V04_0 mute=%0d got=%08h", mute, got);
      $finish;
    end
    wait_word(got, 20000, mute);
    if (!mute) begin
      $display("UART_R2_U17H_FAIL extra after GOLD0 %08h", got);
      $finish;
    end

    for (rnd = 0; rnd < 32; rnd++) begin
      send_word(CLR_CMD);
      expect_one(CLR_ACK, "CLEAR");
      for (i = 0; i < nwords; i++)
        send_word(vec[i]);
      wait_word(got, 2_000_000, mute);
      if (mute || got !== GOLD) begin
        $display("UART_R2_U17H_FAIL V04 mute=%0d got=%08h rnd=%0d", mute, got, rnd);
        $finish;
      end
      wait_word(got, 20000, mute);
      if (!mute) begin
        $display("UART_R2_U17H_FAIL extra after GOLD %08h rnd=%0d", got, rnd);
        $finish;
      end
      $display("U17H rnd=%0d ACK+GOLD", rnd);
    end
    $display("UART_R2_U17H_XSIM_PASS 32/32 ACK_ONLY extra=%0d (not BOARD_PASS)", extra);
    $finish;
  end
endmodule
