// tb_u17_harness_115200.sv — 2x GOLD after CLEAR at board baud. dest=bram.
`timescale 1ns/1ps

module tb_u17_harness_115200;
  ...
  initial begin
    ...
    $display("U17_115200 nwords=%0d DIV=%0d", nwords, DIV);
    ...
    $display("V04_0 mute=%0d got=%08h reason=%02h", mute, got, reason_code);
    if (got === UNSUP) begin $display("FAIL V04_0 UNSUP"); $finish; end
    if (mute || got !== GOLD) begin $display("FAIL V04_0"); $finish; end

    send_word(CLR_CMD);
    wait_word(got, 2_000_000, mute);
    $display("CLEAR2 mute=%0d got=%08h", mute, got);
    if (mute || got !== CLR_ACK) begin $display("FAIL CLEAR2"); $finish; end

    for (i = 0; i < nwords; i++) send_word(vec[i]);
    wait_word(got, 8_000_000, mute);
    $display("V04_1 mute=%0d got=%08h reason=%02h reject=%0b", mute, got, reason_code, load_reject);
    if (got === UNSUP)
      $display("UART_R2_U17_115200_XSIM_FAIL UNSUP (matches board class)");
    else if (!mute && got === GOLD)
      $display("UART_R2_U17_115200_XSIM_PASS GOLD2 dest=bram (board UNSUP not reproduced)");
    else
      $display("UART_R2_U17_115200_XSIM_FAIL got=%08h", got);
    $finish;
  end
endmodule
