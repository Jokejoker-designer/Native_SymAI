...
// UART_R2_U6: q_bytes gated by q_valid; snapshot generation at acceptance.
...
module uart_fe256_host (
  ...
  // U6.4 snapshot. Caller must present a stable generation (pack quiescent).
  input  logic [31:0] active_generation = 32'hFFFF_FFFF,
  output logic [31:0] query_generation_snapshot
);
  ...
  // U6.3: a partial QueryRecord must not be visible on q_bytes.
  always_comb begin : unpack_q
    automatic int k;
    for (k = 0; k < 32; k++)
      q_bytes[k] = q_valid ? qw[k/4][8*(k%4) +: 8] : 8'h0;
  end
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      tx_data <= 32'h0;
      in_ready <= 1'b1;
      query_generation_snapshot <= 32'hFFFF_FFFF;
        ...
        S_RX: begin
          ...
          if (in_valid && in_ready) begin
            ...
            if (wix == 3'h7) begin
              in_ready <= 1'b0;
              q_valid <= 1'b1;
              query_generation_snapshot <= active_generation;
              st <= S_ISSUE;
