@echo off
setlocal
REM Independent UART_R2 build+program. Never writes other arty_d identities.
...
echo ===== SNAPSHOT other-branch hashes =====
python "%PY%" snapshot
if errorlevel 1 exit /b 1

echo ===== SYNTH UART_R2/build only =====
call vivado -mode batch -source "%TCL%\50_synth_uart_r2.tcl" -log "%OUT%\synth.log" -journal "%OUT%\synth.jou"
if errorlevel 1 exit /b 1
python "%PY%" verify
if errorlevel 1 exit /b 1
