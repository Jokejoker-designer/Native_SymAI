// tb_u30_two_v04.sv — U24 board class: GOLD then CLEAR then second V-04 GOLD.
// Dest=mig_ui_bram BAUD=1M. Not BOARD_PASS / not PACK_ABI_24_24_PASS.
`timescale 1ns/1ps

module tb_u30_two_v04;
  ...
  initial begin
    ...
    if (mute || got !== GOLD || (n_p2 > 0 && p2[0] !== BEGINW)) begin
      $display("UART_R2_U30_TWO_V04_XSIM_FAIL got=%08h", got);
      $finish;
    end
    ...
    if (mute || got !== GOLD || (n_p2 > 0 && p2[0] !== BEGINW)) begin
      $display("UART_R2_U30_TWO_V04_XSIM_FAIL r2 got=%08h", got);
      $finish;
    end
    ...
    if (!mute && got === GOLD && (n_p2 == 0 || p2[0] === BEGINW))
      $display("UART_R2_U30_TWO_V04_XSIM_PASS four GOLD through loader-only debug_clear");
    else
      $display("UART_R2_U30_TWO_V04_XSIM_FAIL got=%08h", got);
    $finish;
  end
