// tb_u19_junk_after_gold.sv — after GOLD inject 0-word then CLEAR.
// U18: 0 enters pack_loader → R_UNSUP on CLEAR path or next pack.
// U19: unlocked 0 discarded; CLEAR ACK; next V-04 GOLD.
`timescale 1ns/1ps

module tb_u19_junk_after_gold;
  ...
  initial begin
    ...
    $display("U19_JUNK after0 n_p2=%0d p0=%08h lock=%0b", n_p2,
    ...
    if (got === UNSUP) begin
      $display("UART_R2_U19_JUNK_UNSUP (unlocked 0 reached loader)");
      $finish;
    end
    if (mute || got !== CLR_ACK) begin
      $display("UART_R2_U19_JUNK_FAIL CLEAR2 got=%08h", got);
      $finish;
    end
    ...
    if (!mute && got === GOLD && (n_p2 == 0 || p2[0] === BEGINW))
      $display("UART_R2_U19_JUNK_XSIM_PASS GOLD after post-GOLD 0-word");
    else
      $display("UART_R2_U19_JUNK_XSIM_FAIL got=%08h", got);
    $finish;
  end
endmodule
