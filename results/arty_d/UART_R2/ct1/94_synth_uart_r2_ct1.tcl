# Synth unique CT1 identity. NEW dir D:/FPGA/arty_d/UART_R2/build_ct1.
# Does not overwrite build_u33obs_query / 8fc14f25. No dir_a.mem copy.
...
set want [file normalize {D:/FPGA/arty_d/UART_R2/build_ct1}]
if {$outn ne $want} { r2_fail WRONG_OUT "out=$outn want=$want" }
foreach bad {
  {D:/FPGA/arty_d/UART_R2/build_u33obs_query}
  {D:/FPGA/arty_d/UART_R2/build_u33obs}
  {D:/FPGA/arty_d/UART_R2/build_u33obs_steer}
  {D:/FPGA/arty_d/UART_R2/build_u33obs_rearm}
...
create_project uart_r2_ct1 $out -part xc7a100tcsg324-1 -force
...
read_verilog -sv [file join $r2 ct1 dest_root_cache.sv]
read_verilog -sv [file join $r2 ct1 ct1_lookup_cdc.sv]
read_verilog -sv [file join $r2 ct1 ct1_uart_query.sv]
read_verilog -sv [file join $rtl board clk_arty_mig.sv]
read_verilog -sv [file join $r2 ct1 arty_a7_r2_top_m4_mig_candidate.sv]
...
read_xdc [file join $r2 u33obs_query u33obs_tap_cdc.xdc]
read_xdc [file join $r2 ct1 ct1_lookup_cdc.xdc]
...
set nqcdc [llength [get_cells -quiet u_qcdc/req_tog_reg]]
set ndira [llength [get_files -quiet -filter {NAME =~ *dir_a.mem*}]]
set nexact [llength [get_cells -quiet -hier -filter {REF_NAME =~ exact_directory || ORIG_REF_NAME =~ exact_directory}]]
set fh [open [file join $out BUILD.txt] w]
puts $fh "STATUS=SYNTH_DONE"
puts $fh "OUT=$out"
puts $fh "CLASS=uart_r2_ct1_CANDIDATE"
puts $fh "NOT_OVERWRITE_8fc14f25=YES"
puts $fh "DIR_A_MEM_FILES=$ndira"
puts $fh "EXACT_DIRECTORY_CELLS=$nexact"
puts $fh "TAP_CDC_CELLS=$ntap"
puts $fh "CT1_LOOKUP_CDC_CELLS=$nqcdc"
puts $fh "PROGRAM_PASS=NO"
puts $fh "PACK_ABI_24_24_PASS=NO"
puts $fh "READY_TO_PROGRAM=NO"
puts $fh "PROGRAM=NO"
...
if {$nqcdc < 1} { r2_fail CT1_LOOKUP_CDC_MISSING "u_qcdc/req_tog_reg" }
if {$ndira > 0} { r2_fail DIR_A_MEM_PRESENT "dir_a.mem must not be in CT1 synth" }
puts "uart_r2_ct1_SYNTH_OK path=$out TAP_CDC_CELLS=$ntap QCDC=$nqcdc DIR_A=$ndira"
exit 0
