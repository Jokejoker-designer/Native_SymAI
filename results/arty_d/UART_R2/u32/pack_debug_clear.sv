// pack_debug_clear.sv — UART_R2_U32 CLEAR overlay. Do not patch frozen U20/U30.
// U8 MAG held cdc_rst and ui_req through S_ACK, so debug_clear stayed 1 while ACK
// was on the wire; host V-04 then hit dest-in-reset (U28/U30 Nth n=0 class).
// U32: S_REQ still b-resets TX CDC (ACK path, before debug_clear).
// S_BUSY/S_DROP_B keep TX CDC a+b reset so nack does not replay GOLD hold.
// Do not patch dest_accept / dest_ui_rdy. CANDIDATE.
`timescale 1ns/1ps
...
module pack_debug_clear (
  ...
  logic flush_r;
  wire drop_st = (st == S_DROP) || (st == S_DROP_B);

  assign uart_flush = flush_r;
  assign cdc_rst_100 = cdc_rst_r;
  // Hold TX CDC in reset through BUSY so S_REQ b-reset is not released onto GOLD hold.
  assign tx_busy_rst = (st == S_BUSY) || (st == S_DROP_B);
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      ui_req_r <= (st == S_REQ) || (st == S_CDC) || (st == S_QUIET);
      cdc_rst_r <= (st == S_CDC) || (st == S_QUIET);
      flush_r <= (st == S_CDC) || (st == S_QUIET) || (drop_st && (cnt == 16'h0));
    end
  end
