...
// U31 overlay: qsc false while dest reset; also require mig0 app_rdy/wdf_rdy. PACKAGE live not overwritten.
`timescale 1ns/1ps
...
module pack_mig_bind (
  ...
  input  logic        dest_ui_rdy = 1'b1,
  input  logic        dest_ui_wdf_rdy = 1'b1
);
  ...
  // Overlay U31 only. PACKAGE live bind not overwritten.
  // dest_ui_* is mig0/BRAM accept-ready, NOT mux a_rdy (G_NONE forces a_rdy=0).
  assign pack_quiescent = !loader_busy && !ui_busy && (wr_outstanding == 16'h0)
                       && rst_loc && !debug_clear && dest_ui_rdy && dest_ui_wdf_rdy;
endmodule
