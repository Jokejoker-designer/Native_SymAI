// Scratch TB: dest_stall=1 after first V-04 GOLD, then one CLEAR.
// Scores all UART words ~2 ms. Dest=mig_ui_bram. Not PACK_ABI_24_24_PASS.
// Independent analysis S5: BUSY n=4 vs BUSY+GOLD n=8 vs ACK vs mute.
`timescale 1ns/1ps

module tb_u31_dest_stall_clear;
  ...
  initial begin
    ...
    if (mute || got !== CLR_ACK) begin
      $display("UART_R2_U31_DEST_STALL_XSIM_FAIL CLEAR1 got=%08h mute=%0d", got, mute);
      $finish;
    end
    for (i = 0; i < nwords; i++) send_word(vec[i]);
    wait_word(got, 2_000_000, mute);
    if (mute || got !== GOLD) begin
      $display("UART_R2_U31_DEST_STALL_XSIM_FAIL V04_0 got=%08h mute=%0d", got, mute);
      $finish;
    end
    $display("PHASE4_GOLD qsc_ui=%0b d_rdy=%0b d_wdf=%0b", u_h.qsc_ui, u_h.d_rdy, u_h.d_wdf_rdy);

    dest_stall = 1'b1;
    repeat (64) @(posedge ui_clk);
    repeat (32) @(posedge clk100);
    $display("AFTER_STALL qsc_ui=%0b qsc_100=%0b d_rdy=%0b d_wdf=%0b",
             u_h.qsc_ui, u_h.qsc_100, u_h.d_rdy, u_h.d_wdf_rdy);
    ...
    for (i = 0; i < 8; i++) begin
      ...
      capw[nw] = got;
      nw = nw + 1;
      $display("CLEAR_WORD %0d %08h", nw, got);
    end

    if (nw == 0)
      $display("UART_R2_U31_DEST_STALL_SCORE MUTE");
    else if (nw == 1 && capw[0] === CLR_BUSY)
      $display("UART_R2_U31_DEST_STALL_SCORE BUSY_N4_NO_GOLD");
    else if (nw >= 2 && capw[0] === CLR_BUSY && capw[1] === GOLD)
      $display("UART_R2_U31_DEST_STALL_SCORE BUSY_THEN_GOLD_N8");
    else if (nw == 1 && capw[0] === CLR_ACK)
      $display("UART_R2_U31_DEST_STALL_SCORE ACK");
    else
      $display("UART_R2_U31_DEST_STALL_SCORE OTHER nw=%0d w0=%08h", nw, capw[0]);
    $display("UART_R2_U31_DEST_STALL_XSIM_DONE nw=%0d PACK_ABI_24_24_PASS=NO", nw);
    $finish;
  end
endmodule
