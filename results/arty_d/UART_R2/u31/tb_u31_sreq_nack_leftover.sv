// Scratch TB: live leftover race SAMPLE qsc_100=1 → S_REQ ui_req → dest_stall nack → BUSY.
// Expect CDC b-reset replay GOLD without force. Dest=mig_ui_bram. Not PACK_ABI_24_24_PASS.
`timescale 1ns/1ps

module tb_u31_sreq_nack_leftover;
  ...
  initial begin
    ...
    if (mute || got !== CLR_ACK) begin
      $display("UART_R2_U31_SREQ_NACK_FAIL CLEAR1 got=%08h", got);
      $finish;
    end
    for (i = 0; i < nwords; i++) send_word(vec[i]);
    wait_word(got, 2_000_000, mute);
    if (mute || got !== GOLD) begin
      $display("UART_R2_U31_SREQ_NACK_FAIL V04_0 got=%08h", got);
      $finish;
    end
    ...
    if (nw == 0)
      $display("UART_R2_U31_SREQ_NACK_SCORE MUTE");
    else if (nw == 1 && capw[0] === CLR_BUSY)
      $display("UART_R2_U31_SREQ_NACK_SCORE BUSY_N4_NO_GOLD");
    else if (nw >= 2 && capw[0] === CLR_BUSY && capw[1] === GOLD)
      $display("UART_R2_U31_SREQ_NACK_SCORE BUSY_THEN_GOLD_N8");
    else if (nw == 1 && capw[0] === CLR_ACK)
      $display("UART_R2_U31_SREQ_NACK_SCORE ACK");
    else
      $display("UART_R2_U31_SREQ_NACK_SCORE OTHER nw=%0d w0=%08h", nw, capw[0]);
    $display("UART_R2_U31_SREQ_NACK_XSIM_DONE PACK_ABI_24_24_PASS=NO");
    $finish;
  end
endmodule
