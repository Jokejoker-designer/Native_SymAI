// tb_u31_two_v04.sv — U24 board class: GOLD then CLEAR then second V-04 GOLD.
// Dest=mig_ui_bram BAUD=1M. Not BOARD_PASS / not PACK_ABI_24_24_PASS.
`timescale 1ns/1ps

module tb_u31_two_v04;
  ...
  initial begin
    ...
    send_word(CLR_CMD);
    wait_word(got, 400000, mute);
    if (mute || got !== CLR_ACK) begin $display("FAIL CLEAR2 %08h", got); $finish; end

    cap2 = 1'b1;
    for (i = 0; i < nwords; i++) send_word(vec[i]);
    wait_word(got, 2_000_000, mute);
    $display("V04_1 mute=%0d got=%08h first_p=%08h empty=%0b", mute, got,
             (n_p2 > 0) ? p2[0] : 32'h0, u_h.fifo_empty);
    if (mute || got !== GOLD || (n_p2 > 0 && p2[0] !== BEGINW)) begin
      $display("UART_R2_U31_TWO_V04_XSIM_FAIL got=%08h", got);
      $finish;
    end
    ...
    send_word(CLR_CMD);
    wait_word(got, 400000, mute);
    if (mute || got !== CLR_ACK) begin $display("FAIL CLEAR3 %08h", got); $finish; end
    cap2 = 1'b1;
    for (i = 0; i < nwords; i++) send_word(vec[i]);
    wait_word(got, 2_000_000, mute);
    $display("V04_2 mute=%0d got=%08h first_p=%08h", mute, got, (n_p2 > 0) ? p2[0] : 32'h0);
    if (mute || got !== GOLD || (n_p2 > 0 && p2[0] !== BEGINW)) begin
      $display("UART_R2_U31_TWO_V04_XSIM_FAIL r2 got=%08h", got);
      $finish;
    end
    ...
    send_word(CLR_CMD);
    wait_word(got, 400000, mute);
    if (mute || got !== CLR_ACK) begin $display("FAIL CLEAR4 %08h", got); $finish; end
    cap2 = 1'b1;
    for (i = 0; i < nwords; i++) send_word(vec[i]);
    wait_word(got, 2_000_000, mute);
    $display("V04_3 mute=%0d got=%08h first_p=%08h empty=%0b", mute, got,
             (n_p2 > 0) ? p2[0] : 32'h0, u_h.fifo_empty);
    if (!mute && got === GOLD && (n_p2 == 0 || p2[0] === BEGINW))
      $display("UART_R2_U31_TWO_V04_XSIM_PASS four GOLD through loader-only debug_clear");
    else
      $display("UART_R2_U31_TWO_V04_XSIM_FAIL got=%08h", got);
    $finish;
  end
