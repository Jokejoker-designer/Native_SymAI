// pack_debug_clear.sv — UART_R2_U20 CLEAR.
// U19 FAIL_BOARD exclusive: CLEAR ACK then V-04 n=0. WAIT_AFTER_ACK_S=0.
// U18/U19: uart_flush stays 1 for the whole S_DROP|S_DROP_B while ui_ack falls
// (up to TO_N cycles). That destroys a following command already on the wire
// (V-04 after ACK, or CLEAR after GOLD). Must NOT flush S_REQ (U16) or S_ACK (U10).
// U20: flush S_CDC|S_QUIET plus ONE cycle of S_DROP/S_DROP_B (cnt==0) to destroy
// ACK-parked RX, then flush=0 so a following word is parked by hold, not wiped.
// MAG cdc_rst = S_CDC|S_QUIET|S_ACK (U8). Do not edit frozen u19. CANDIDATE.
`timescale 1ns/1ps
