"""U26 board campaign: MARK 2s, exact n=4 ACK/GOLD, then Pack24.
...
u9mod.OUT = Path(r"D:\FPGA\arty_d\UART_R2\results\PACK24_U26")
OUT = u9mod.OUT
RAW = OUT / "RAW_UART"
BIT = Path(r"D:\FPGA\arty_d\UART_R2\build_u26\uart_r2_u26_candidate.bit")
WANT = "8f5471a7d2da35dacfec42ede3e1b9a9b19e7f90e3e23a4ba42c94b267df3375"
TSV = Path(
...
def main() -> int:
    ...
    try:
        if mode in {"p4", "p4p5"}:
            ...
            c1 = send_clear(ser, recs, "CLEAR1")
            if c1.get("n", 0) == 0:
                c1 = send_clear(ser, recs, "CLEAR1_RETRY")
            if c1.get("n", 0) == 0:
                ser = reopen_mark(ser, port)
                recs.append({"step": "REOPEN_AFTER_N0", "port": port})
                print("REOPEN_AFTER_N0")
                c1 = send_clear(ser, recs, "CLEAR1_REOPEN")
        ...
        if mode in {"p5", "p4p5"}:
            ...
            for rnd in range(24):
                c = send_clear(ser, recs, "CLEAR", rnd)
                if c.get("n", 0) == 0:
                    c = send_clear(ser, recs, "CLEAR_RETRY", rnd)
                if c.get("n", 0) == 0:
                    ser = reopen_mark(ser, port)
                    recs.append({"step": "REOPEN_AFTER_N0", "round": rnd, "port": port})
                    print("REOPEN_AFTER_N0", rnd)
                    c = send_clear(ser, recs, "CLEAR_REOPEN", rnd)
