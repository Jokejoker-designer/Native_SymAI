...
// Observe-only. Overflow OR. PROGRAM=NO.
`timescale 1ns/1ps
...
module pack_obs_hops (
  ...
  pack_obs_lane u_uart (
    ...
    .overflow(ov_u), .n_ev(n_uart), .wr_ptr(unused_wr[0]),
  ...
  pack_obs_lane u_fifo_wr (
    ...
    .overflow(ov_fw), .n_ev(n_fifo_wr), .wr_ptr(unused_wr[1]),
    .rd_en(1'b0), .rd_addr(8'h0), .rd_data()
  );
  pack_obs_lane u_fifo_rd (
    .clk(clk100), .rst_n(rst100_n), .arm(arm_100), .armed(armed_100),
    .freeze(freeze_100), .clr_event, .epoch_id,
    .fire(fifo_rd_fire), .data(fifo_rd_data), .flags(16'h0004),
    .overflow(ov_fr), .n_ev(n_fifo_rd), .wr_ptr(unused_wr[2]),
    .rd_en(1'b0), .rd_addr(8'h0), .rd_data()
  );
  pack_obs_lane u_cdc_a (
    ...
    .overflow(ov_ca), .n_ev(n_cdc_a), .wr_ptr(unused_wr[3]),
    .rd_en(1'b0), .rd_addr(8'h0), .rd_data()
  );
  pack_obs_lane u_cdc_b (
    ...
    .overflow(ov_cb), .n_ev(n_cdc_b), .wr_ptr(unused_wr[4]),
    .rd_en(1'b0), .rd_addr(8'h0), .rd_data()
  );
  pack_obs_lane u_load (
    ...
    .overflow(ov_ld), .n_ev(n_load), .wr_ptr(unused_wr[5]),
