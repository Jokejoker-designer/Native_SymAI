// pack_debug_clear.sv — UART_R2_U18 CLEAR.
// U17 FAIL_BOARD: GOLD then CLEAR ACK then V-04 R_UNSUP 0200075a.
// U8 MAG cdc_rst = S_CDC|S_QUIET|S_ACK (do not extend through S_DROP).
// U8/U17 flush = S_CDC|S_QUIET only. During S_ACK hold=1 so UART RX can
// park a complete word (w_valid/stop_hold) that MARK/U11 will not destroy.
// That parked word is emitted on S_IDLE → opcode 0 / phase-shift → R_UNSUP.
// U18: uart_flush also S_DROP|S_DROP_B AFTER ACK handshake. Destroys parked
// RX/FIFO state. Must NOT flush S_REQ (U16 n=0) or S_ACK (U10 abort / ACK mute).
// Do not edit frozen u17. CANDIDATE. Not BOARD_PASS.
`timescale 1ns/1ps
...
module pack_debug_clear (
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      flush_r <= (st == S_CDC) || (st == S_QUIET) || (st == S_DROP) || (st == S_DROP_B);
    end
  end
