"""U18 board campaign: MARK 2s, exact n=4 ACK/GOLD, then Pack24.
...
u9mod.OUT = Path(r"D:\FPGA\arty_d\UART_R2\results\PACK24_U18")
OUT = u9mod.OUT
RAW = OUT / "RAW_UART"
BIT = Path(r"D:\FPGA\arty_d\UART_R2\build_u18\uart_r2_u18_candidate.bit")
WANT = "aca343792c09feaaef3ab5dcbb6326f784d7ef80ac518bff15b32513a7238949"
TSV = Path(
...
MARK_S = 2.0
WAIT_AFTER_ACK_S = 0.05
WAIT_AFTER_GOLD_S = 0.05
...
def main() -> int:
    ...
    if not port:
        print("NO_COM")
        return 2
    # COM-open dummy: MARK recovery without a command, then a real session.
    ...
    try:
        if mode in {"p4", "p4p5"}:
            ...
            if c1.get("n", 0) == 0:
                send_words(ser, [CLR_CMD])
                c1 = read_raw_stamped(ser, 3.0)
                recs.append({"step": "CLEAR1_RETRY", **c1})
                print("CLEAR1_RETRY", c1["sub"], c1["n"], c1.get("raw_hex"))
