// tb_u18_drop_flush.sv — U18 CLEAR flush lifecycle.
// S_CDC|S_QUIET flush=1; S_ACK flush=0 cdc_rst=1; S_DROP flush=1 cdc_rst=0.
// Not BOARD_PASS.
`timescale 1ns/1ps

module tb_u18_drop_flush;
  ...
  initial begin
    ...
    if (cdc_rst_100) tfail("S_DROP cdc_rst still 1");
    else tpass("S_DROP cdc_rst=0");
    if (!uart_flush) tfail("S_DROP flush=0 (U18 requires post-ACK RX destroy)");
    else tpass("S_DROP flush=1");
    c = 0;
    ...
    if (fail == 0)
      $display("UART_R2_U18_DROP_FLUSH_XSIM_PASS pass=%0d", pass);
    else
      $display("UART_R2_U18_DROP_FLUSH_XSIM_FAIL fail=%0d pass=%0d", fail, pass);
    $finish;
  end
endmodule
