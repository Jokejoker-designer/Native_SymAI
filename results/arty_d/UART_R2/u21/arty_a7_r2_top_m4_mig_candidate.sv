// UART_R2_U21 overlay. PACKAGE live top NOT overwritten.
// U20 FAIL_BOARD: GOLD then Phase5 CLEAR n=0. U16 correlated TX-level flush with mute.
// U21: uart_tx_word.flush=0; clr_ack_ready=mux_ready; FIFO flush also on pack_lock fall.
// RX/FIFO still U20 one-cycle DROP flush. U19 steer_pack kept. TX CDC B ~clr_ui_req.
...
module arty_a7_r2_top_m4_mig_candidate (
  ...
  logic fifo_wr_ready, f_valid, f_ready, fifo_empty, pack_lock, pack_lock_d, fifo_flush;
  logic [31:0] f_data;
  ...
  always_ff @(posedge clk100 or negedge rst100_n) begin
    if (!rst100_n)
      ...
      pack_lock_d <= pack_lock;
  end
  assign fifo_flush = uart_flush || (pack_lock_d && !pack_lock);
