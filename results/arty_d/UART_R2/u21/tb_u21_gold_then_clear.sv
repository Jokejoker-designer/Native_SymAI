// tb_u21_gold_then_clear.sv — board U20 first-fail: GOLD then immediate CLEAR.
// Dest=mig_ui_bram BAUD=1M. Not BOARD_PASS / not product mig0.
`timescale 1ns/1ps

module tb_u21_gold_then_clear;
  ...
  initial begin
    ...
    $display("U21_G2C nwords=%0d DIV=%0d", nwords, DIV);

    send_word(CLR_CMD);
    wait_word(got, 400000, mute);
    if (mute || got !== CLR_ACK) begin
      $display("UART_R2_U21_G2C_FAIL CLEAR1 mute=%0d got=%08h", mute, got);
      $finish;
    end
    ...
    if (mute || got !== GOLD) begin
      $display("UART_R2_U21_G2C_FAIL V04_0 mute=%0d got=%08h", mute, got);
      $finish;
    end
    ...
    if (mute) begin
      $display("UART_R2_U21_G2C_FAIL CLEAR2 n=0 (board U20 class)");
      $finish;
    end
    if (got === BUSY) begin
      $display("UART_R2_U21_G2C_FAIL CLEAR2 BUSY");
      $finish;
    end
    if (got === UNSUP) begin
      $display("UART_R2_U21_G2C_FAIL CLEAR2 UNSUP");
      $finish;
    end
    if (got !== CLR_ACK) begin
      $display("UART_R2_U21_G2C_FAIL CLEAR2 got=%08h", got);
      $finish;
    end
    ...
    wait_word(got, 2_000_000, mute);
    if (got === UNSUP)
      $display("UART_R2_U21_G2C_XSIM_PASS CLEAR2 ACK; V04_1 UNSUP (U17 board class, bram)");
    else if (!mute && got === GOLD)
      $display("UART_R2_U21_G2C_XSIM_PASS GOLD then CLEAR ACK then GOLD2 dest=bram");
    else
      $display("UART_R2_U21_G2C_FAIL V04_1 mute=%0d got=%08h", mute, got);
    $finish;
  end
endmodule
