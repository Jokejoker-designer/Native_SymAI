// tb_u29_leftover_op01.sv — U20 board class: leftover opcode 0x01 after GOLD.
...
module tb_u29_leftover_op01;
  ...
  initial begin
    ...
    if (n_p2 > 0 && p2[0] !== BEGINW) begin
      $display("UART_R2_U29_LEFTOVER_UNSUP junk reached loader p0=%08h", p2[0]);
      $finish;
    end
    ...
    if (got === UNSUP) begin
      $display("UART_R2_U29_LEFTOVER_UNSUP on CLEAR");
      $finish;
    end
    if (mute || got !== CLR_ACK) begin
      $display("UART_R2_U29_LEFTOVER_FAIL CLEAR2 got=%08h", got);
      $finish;
    end
    ...
    if (!mute && got === GOLD && (n_p2 == 0 || p2[0] === BEGINW))
      $display("UART_R2_U29_LEFTOVER_XSIM_PASS GOLD after unlocked opcode-01 junk");
    else
      $display("UART_R2_U29_LEFTOVER_XSIM_FAIL got=%08h", got);
    $finish;
  end
endmodule
