...
set XD=D:\FPGA\arty_d\pack_debug_clear\xsim
...
call xvlog -sv -i "%PA%" -i "%PA%\out" ^
  ...
  "%ROOT%\tb\native_ai\loader\tb_pack_abi24_clear.sv"
if errorlevel 1 exit /b 1
call xelab tb_pack_abi24_clear -snapshot tb_pa24_clr -timescale 1ns/1ps
if errorlevel 1 exit /b 1
echo run -all> "%XD%\run_all.tcl"
echo exit>> "%XD%\run_all.tcl"
call xsim tb_pa24_clr -tclbatch run_all.tcl -log "%XD%\xsim.log"
if errorlevel 1 exit /b 1
