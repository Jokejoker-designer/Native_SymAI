...
set TB=%ROOT%\tb\native_ai\board
set XD=D:\FPGA\arty_d\pack_debug_clear\xsim_uart
...
call xvlog -sv ^
  ...
  "%TB%\tb_pack_debug_clear.sv"
if errorlevel 1 exit /b 1
call xelab tb_pack_debug_clear -snapshot snap_clr -timescale 1ns/1ps
if errorlevel 1 exit /b 1
echo run -all> "%XD%\run_all.tcl"
echo exit>> "%XD%\run_all.tcl"
call xsim snap_clr -tclbatch run_all.tcl -log "%XD%\xsim.log"
if errorlevel 1 exit /b 1
