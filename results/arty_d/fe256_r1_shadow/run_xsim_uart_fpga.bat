...
set RTL=%ROOT%\rtl\native_ai
set XD=D:\FPGA\arty_d\fe256_r1_shadow\xsim_uart_fpga
...
copy /Y "%ROOT%\tb\native_ai\loader\vectors\v1_valid.mem" "%XD%\" >nul
copy /Y "%ROOT%\tb\native_ai\loader\vectors\v2_bad_magic.mem" "%XD%\" >nul
call xvlog -sv -i "%ROOT%\verification\fe256" ^
  ...
  "%ROOT%\verification\fe256\tb_r2_fe256_r1_uart_fpga.sv"
if errorlevel 1 exit /b 1
call xelab tb_r2_fe256_r1_uart_fpga -snapshot tb_uart_fpga -timescale 1ns/1ps
if errorlevel 1 exit /b 1
echo run -all> "%XD%\run_all.tcl"
echo exit>> "%XD%\run_all.tcl"
call xsim tb_uart_fpga -tclbatch run_all.tcl -log "%XD%\xsim.log"
if errorlevel 1 exit /b 1
