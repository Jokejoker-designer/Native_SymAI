// H-ILA-A debug identity. Same H functional modules + observe/dump.
// BASIC license cannot create_debug_core; this is the probe substitute.
// Do not overwrite identity H bit cf62102f or freeze DCPs.
// Not BOARD_PASS / PROGRAM_PASS / PACK_ABI_24_24_PASS / TIMING_PASS / MIG_PASS.
`timescale 1ns/1ps

module arty_a7_r2_top_m4_mig_clear_ila_a (
  ...
  logic dbg_rx_d;
  logic [3:0] dbg_bitn;
  logic cap_captured, cap_false, cap_have_word, cap_wv0, cap_fifo0;
  logic [7:0] cap_sh0, cap_sh1, cap_sh2, cap_sh3;
  logic [3:0] cap_n;
  logic [1:0] cap_bix_arm;
  logic [9:0] cap_wire10;
  logic [31:0] cap_word;
  ...
  h_ila_a_cap u_cap (
    ...
    .n_bytes(cap_n), .bix_at_arm(cap_bix_arm), .wire10(cap_wire10),
    .false_start(cap_false), .have_word(cap_have_word), .first_word(cap_word),
    .wv_at_sh0(cap_wv0), .fifo_wr_at_sh0(cap_fifo0)
  );
  ...
  always_ff @(posedge clk100 or negedge rst100_n) begin
    if (!rst100_n)
      ...
      dump_go <= st_valid_100 && st_ready_100;
  end
