// g2_two_effects_r1.sv
// Generation 2 selects the evidence and the command.
// The same command id is sensed twice. A code equal to the primitive does not enter FEM.
// A different code does, and the next proposal changes. The evidence stays generation 2.
// Does not edit g2_observe_r1, primitive_executor_r1, fem_lifecycle, or qstar_select.
`timescale 1ns/1ps

module g2_two_effects_r1 (
  input  logic        clk,
  input  logic        rst_n,
  input  logic        s_valid,
  ...
  command_effect_hold_r1 u_eff (
    .clk, .rst_n, .product_done, .command_valid, .command_primitive, .command_id,
    .effect_table, .sense_req, .effect_valid, .effect_result, .effect_command_id
  );
