"""U33TAP CDC-impl silicon capture. Not overlay U33. Not PACK_ABI_24_24_PASS.

Identity: uart_r2_u33tap_cdc_candidate.bit SHA eb99ac69... (not d448544f).
CELL_DUP4 leftover BEGIN+V-04 is the dump-align test. Reopen once if CLEAR n=0.
"""
...
WANT = "eb99ac69f07e582c1ac4b1a1da876bfa1a381310133b9fe7c7605bf9e766166b"
BIT = Path(r"D:\FPGA\arty_d\UART_R2\build_u33tap_cdc\uart_r2_u33tap_cdc_candidate.bit")
BAN = {
...
GOLD = 0x010000A5
MARK_S = 2.0
OUT = Path(r"D:\FPGA\arty_d\UART_R2\results\U33TAP_CDC_CAPTURE_20260920")
...
def classify_tap(words: list[int]) -> dict:
    ...
    return {
        ...
        "p1_is_xsim_begin": p1 == BEGINW,
        "p1_is_old_silicon": p1 == 0x414E0080,
    }
...
def main() -> int:
    ...
    try:
        ...
        ser.close()
    if cells[-1].get("verdict") == "MUTE_CLEAR":
        time.sleep(1.0)
        ser = open_mark(port)
        try:
            cells.append(run_cell(ser, "CELL_DUP4_REOPEN", True, v04))
        finally:
            ser.close()
    out = {
        "iso": now_iso(),
